...
class Subscriber : public ISubscriber<TListener> {
private:
...
public:
    void subscribe(std::reference_wrapper<TListener> listener) override
    {
        auto iter = std::find_if(
            ...
            [&listener](std::reference_wrapper<TListener> item) { return &item == &listener; });
    ...
    void unsubscribe(std::reference_wrapper<TListener> listener) override
    {
        auto iter = std::find_if(
            mListeners.begin(), mListeners.end(),
            [&listener](std::reference_wrapper<TListener> item) { return &listener == &item; });
        if (iter != mListeners.end()) {
